...
def load_jddc(dirname, tokenizer, lite=1):
    ...
    if lite:
        name = name + '_lite'
    # if os.path.exists(f'dataset/jddc/tokenized/{name}.pkl'):
    #     return read_pkl(f'dataset/jddc/tokenized/{name}.pkl')
    ...
    write_pkl(data, f'dataset/jddc/tokenized/{name}.pkl')
    return data
...
def train(fold=0, data_name='dstc8', model_name='HiGRU+ATTN'):
    ...
    if model_name == 'HiGRU+ATTN':
        ...
        model = ClassModel(backbone=BERTBackbone(layers_used=2, name='bert-base-chinese'), class_num=[action_num])
        model = model.cuda()
